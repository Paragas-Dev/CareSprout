...
function updatePostButtonState(editorContent = null) {
    const textContent = stripHtml(
        editorContent !== null
            ? editorContent
            : CKEDITOR.instances["announcement-text"]
            ? CKEDITOR.instances["announcement-text"].getData()
            : textarea.value
    );
...
const announcementModalBackdrop = document.getElementById(
    "announcement-modal-backdrop"
);

const announcementForm = document.getElementById("new-announcement-form");
const textarea = document.getElementById("announcement-text");
const attachedFilesContainer = document.getElementById(
...
async function uploadFileToCloudinary(file, origin = "local") {
    ...
    if (typeof cloudinary === "undefined" || !cloudName || !uploadPreset) {
        uploadStatus.textContent =
            "Cloudinary configuration or script not loaded.";
    ...
    try {
        ...
        uploadStatus.textContent = `Uploaded ${
            uploadedAsset.original_filename || uploadedAsset.public_id
        } to Cloudinary!`;
    ...
    insertYoutubeBtn.addEventListener("click", () => {
        ...
        if (videoUrl) {
            ...
            if (videoIdMatch && videoIdMatch[1]) {
                ...
                fetch(`/api/youtube-title?videoId=${videoId}`)
                    .then((response) => {
                        if (!response.ok) {
                            throw new Error(
                    ...
                    .catch((error) => {
                        console.error(
                            "Error fetching YouTube video metadata:",
                            error
                        );
                        alert(
                            "Failed to retrieve video title. Please check the video ID or try again later."
                        );
            ...
            //     try {
            //         const videoMetadata = youtube.get_metadata(urls=[`https://www.youtube.com/watch?v=${videoId}`]);
            //         if (videoMetadata && videoMetadata.length > 0) {
            //             videoTitle = videoMetadata[0].title;
            //         }
            //     } catch (error) {
            //         console.error("Error fetching YouTube video metadata:", error);
            //         alert("Failed to get video title. Using default name.");
            //     }
...
function addAttachmentToPreview(attachment) {
    attachment.id =
        "attachment-" + Date.now() + Math.random().toString(36).substr(2, 9);
...
window.addEventListener("firebaseReady", function () {
    ...
    var myWidget = cloudinary.createUploadWidget(
        ...
        (error, result) => {
            if (!error && result && result.event === "success") {
                ...
                console.log(
                    "Done uploading file via widget to Cloudinary!",
                    uploadedAsset
                );
    ...
    auth.onAuthStateChanged((user) => {
        if (user) {
            ...
            currentUserName =
                document.body.dataset.authUserName || "Default Admin Name";
            console.log(
                "No Firebase User. Using Laravel fallback (if available):",
                currentUserName
            );
        }
    });
    ...
    db.collection("lessons")
        ...
        .then((doc) => {
            ...
            document.addEventListener("click", (event) => {
                if (
                    lessonInfoPopup.classList.contains("show") &&
                    !lessonInfoPopup.contains(event.target) &&
                    event.target !== lessonInfoIcon
                ) {
    ...
    if (announcementForm) {
        announcementForm.onsubmit = function (e) {
            e.preventDefault();
            const editorData = CKEDITOR.instances["announcement-text"]
                ? CKEDITOR.instances["announcement-text"].getData()
                : textarea.value;
            ...
            if (!plainText && currentAttachments.length === 0) {
                alert(
                    "Please enter some text or add an attachment to your announcement."
                );
                return;
            }
        ...
        db.collection("lessons")
            ...
            .then((querySnapshot) => {
                ...
                if (querySnapshot.empty) {
                    postsContainer.innerHTML =
                        '<p style="text-align: center; color: #777;">No announcements yet. Be the first to post!</p>';
                }
                ...
                querySnapshot.forEach((doc) => {
                    ...
                    if (postAttachments.length > 0) {
                        attachmentsHtml +=
                            '<div class="post-attachments-display">';
          ...
          `;
                    ...
                    const optionsIcon = postDiv.querySelector(
                        ".post-options .fa-ellipsis-v"
                    );
                    const dropdownMenu = postDiv.querySelector(
                    ...
                    const commentInput =
                        postDiv.querySelector(".comment-input");
                    const sendCommentBtn =
                        postDiv.querySelector(".send-comment-btn");
                    const latestCommentDisplay = postDiv.querySelector(
                    ...
                    db.collection("lessons")
                        ...
                        .then((commentSnapshot) => {
                            if (!commentSnapshot.empty) {
                                const latestComment =
                                    commentSnapshot.docs[0].data();
                                const commentTime = latestComment.timestamp
                                    ? new Date(
                                          latestComment.timestamp.toDate()
                                      ).toLocaleString()
                                    : "Just now";
                    ...
                    document
                        ...
                        .forEach((openDropdown) => {
                            const optionsContainer =
                                openDropdown.closest(".post-options");
                            if (
                                optionsContainer &&
    ...
    function closeAllStudentDropdowns(event) {
        document
            .querySelectorAll(".person-options-dropdown.show")
            .forEach((openDropdown) => {
                const optionsContainer =
                    openDropdown.closest(".person-options");
                if (
                    optionsContainer &&
                    !optionsContainer.contains(event.target)
                ) {
    ...
    function removeStudent(studentUid, studentName) {
        if (
            ...
            db.collection("lessons")
                ...
                .then(() => {
                    console.log(
                        `Student ${studentName} (${studentUid}) removed successfully!`
                    );
                    renderPeopleSection();
                })
                .catch((error) => {
        ...
        db.collection("lessons")
            ...
            .then((doc) => {
                if (doc.exists) {
                    ...
                    const creatorName =
                        lessonData.createdBy && lessonData.createdBy.name
            ...
            .then((querySnapshot) => {
                if (querySnapshot.empty) {
                    ...
                    querySnapshot.forEach((doc) => {
                        ...
                        const optionsIcon = studentRow.querySelector(
                            ".person-options .fa-ellipsis-v"
                        );
                        const dropdownMenu = studentRow.querySelector(
                            ".person-options-dropdown"
                        );
    ...
    document.querySelectorAll(".lessons-tabs .tab").forEach((tab) => {
        tab.addEventListener("click", function () {
            document
                .querySelectorAll(".lessons-tabs .tab")
